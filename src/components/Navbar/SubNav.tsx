		...
		<div className="px-5 py-3 relative w-full border-b border-blue_gray-100">
			<div className="w-full flex flex-col md:flex-row items-center">
				...
				<div className="w-3/4 flex items-center justify-between">
					<List className="w-full md:w-[55%] flex items-center justify-between" orientation="horizontal">
						{entities?.map((el, index) => (
							<Fragment key={index}>
								<EntityOverview title={el.title} src={el.src} value={el.value} />
								{entities.length - 1 !== index && <Line className="self-center h-11 bg-gray-300 w-px" />}
							</Fragment>
						))}
					</List>
					...
					<div className="flex items-center gap-5">
						<Button
							...
							shape="rounded"
							variant="bg-indigo-800 shadow-bs text-white-A700"
						>
							save
						</Button>
						<Button
							...
							shape="rounded"
							variant="bg-white-A700 shadow-bs text-indigo-800"
						>
							export
						</Button>
					</div>
